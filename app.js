  ...
  if (ext === ".docx") {
    fs.readFile(caminho, async (err, data) => {
      ...
      try {
        ...
        // Chamada da IA
        let feedbackIA = await executar(textoPuro.value || textoPuro);

        // Aqui faz o replace dos ** para <strong>
        let feedbackIAFormatado = feedbackIA.replace(/\*\*(.*?)\*\*/g, '</p><p><strong>$1</strong>');
        

        res.render("index", {
          conteudoHtml: html,
          feedbackIA: feedbackIAFormatado
        });
      } catch (error) {
